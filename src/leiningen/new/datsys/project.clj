...
;; https://github.com/reagent-project/reagent/issues/275
  ...
  :main ^:skip-aot {{ns-name}}.run
  :cljsbuild {:builds
              {:client
               {:source-paths ["src/cljc" "src/cljs"]
                :compiler {:main {{ns-name}}.app
  ...
  :figwheel {:server-port {{figwheel-port}}                ;; default. overwritten by datsys config anyways
             ;; ^ was nrepl-port in the template branch?
             ...
             :dev [:dev-config
                   {:dependencies [[alembic "0.3.2"]
                    ...
                    :cljsbuild {:builds {:client {:source-paths ["dev/cljs" "dev/cljc"]
                                                  :figwheel {:on-jsload "{{ns-name}}.start/on-js-reload"}}}}
